...
final class PhotoPresenter extends ChangeNotifier {
  ...
  List<Uint8List> get selectedImages => _selectedImageData;

  Uint8List? get preview => selectedImages.lastOrNull;
  ...
  onSelectedImage(int index) async {
    final imageData = await _currentAlbumImages[index].originBytes;
    if (imageData != null) {
      if (_selectedImageIndexList.contains(index)) {
        _selectedImageIndexList = List.from(_selectedImageIndexList..remove(index));
        _selectedImageData = List.from(_selectedImageData..remove(imageData));
      } else {
        _selectedImageIndexList = List.from(_selectedImageIndexList + [index]);
        _selectedImageData = List.from(_selectedImageData + [imageData]);
      }
      notifyListeners();
    }
  }
